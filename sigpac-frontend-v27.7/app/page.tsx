...
export default function Home() {
  ...
  // Calcula el total usado de un producto en todos los tratamientos (suma dosis x superficie de cada parcela)
      ...
      <div style={{ display: 'flex', background: 'var(--surface)', borderBottom: '1px solid var(--border)', flexShrink: 0 }}>
        ...
        {(['mapa', 'mis_parcelas', 'tratamientos', 'personal'] as Pestaña[]).map(tab => (
          <button
            ...
            style={{
              padding: isMobile ? '10px 12px' : '12px 20px', border: 'none', background: 'transparent',
              fontFamily: 'var(--mono)', fontSize: isMobile ? 10 : 12, fontWeight: 700, cursor: 'pointer',
              color: pestana === tab ? 'var(--green)' : 'var(--muted)',
              borderBottom: `2px solid ${pestana === tab ? 'var(--green)' : 'transparent'}`,
              letterSpacing: isMobile ? '0.03em' : '0.06em', transition: 'all 0.15s',
              whiteSpace: 'nowrap',
            }}
          >
        ...
        {pestana === 'mapa' && (
          <div style={{ display: 'flex', width: '100%', height: '100%', position: 'relative' }}>
            ...
            {isMobile && (
              <div
                onClick={() => {
                  if (!parcGeojson && !panelAbierto) { setSeleccionando(true); setPanelAbierto(false) }
                  else setPanelAbierto(p => !p)
                }}
                style={{ padding: '14px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'pointer', flexShrink: 0 }}
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  ...
                  <span style={{ fontFamily: 'var(--mono)', fontSize: 12, fontWeight: 700, color: 'var(--green)' }}>
                    {parcGeojson ? (parcelaInfo?.origen === 'dibujado_mano' ? 'Parcela dibujada' : `Pol. ${parcelaInfo?.poligono || ''} - Par. ${parcelaInfo?.parcela || ''}`) : 'Toca el mapa para seleccionar'}
                  </span>
                </div>
                <span style={{ color: 'var(--muted)', fontSize: 18 }}>{panelAbierto ? '▾' : '▸'}</span>
              </div>
            )}
            <div style={{ flex: 1, overflowY: 'auto', padding: isMobile ? '0 16px 16px' : '0', display: 'flex', flexDirection: 'column', gap: 14, display: panelAbierto || !isMobile ? 'flex' : 'none' }}>
                    ...
                    {climaAbierto && (
                      <div style={{ padding: '12px 14px' }}>
                        ...
                        {climaDatos && !climaCargando && (
                          <>
                            {climaDatos.estacion && (
                              <div style={{ fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--muted)', marginBottom: 8, padding: '4px 0' }}>
                                Estacion: <span style={{ color: 'var(--text)', fontWeight: 600 }}>{climaDatos.estacion}</span>
                              </div>
                            )}
